...
class PictureCut{
	...
	private function populateFileFromStream($fileStream){
		...
		if($this->imageNameRandom == "true"){
			...
			$newName = dechex(round(rand(0,mt_getrandmax()))).".".$this->fileType;
	...
	public function upload(){
		...
		try {
                if (strpos($this->currentFile, 'webroot/webroot') !== false) {
                    $this->currentFile = str_replace('webroot/webroot', 'webroot', $this->currentFile);
                }
